...
from context_summary import build_summary_prompt, build_context_summary
...
def summarize_memories(memory_list, user_id, mode="neutral"):
    ...
    processed = [entry.strip().removeprefix("User:").strip() for entry in memory_list]
    prompt = build_summary_prompt(processed, user_id, mode=mode)
    ...
    try:
        ...
        # Curiosity fallback logic
        if not parsed["consider_next"] or len(parsed["consider_next"]) < 10:
            latest_message = processed[-1]
            past_topics = []  # optional: pull from thread metadata if needed
            curiosity_question = generate_curiosity_prompt(latest_message, past_topics)
            parsed["consider_next"] = curiosity_question
